...
static waavs::StopWatch gSVGAppClock;
...
{
    static std::unique_ptr<waavs::Recorder> gRecorder = std::make_unique<waavs::Recorder>(nullptr);
...
{
    auto fh = waavs::FontHandler::getFontHandler();
    if (nullptr == fh)
    {
        return false;
    }
