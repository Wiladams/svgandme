...
namespace waavs {
	...
	struct PathCmdPrinter
	{
		...
		void execCommand(uint8_t kind, const double* args, int iteration)
		{
			...
			if (kind < 128 && commandTable[kind]) 
			{
				(this->*(commandTable[kind]))(args, iteration);
			}
		...
		{
			execCommand(cmdState.fSegmentKind, cmdState.args, cmdState.iteration);
		}
