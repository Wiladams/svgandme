    ...
    float k4) noexcept
{
    ...
    auto to_i32 = [](double v) noexcept -> int32_t
        {
            if (v > 2147483647.0)
                return 2147483647;
            if (v < -2147483648.0)
    ...
    fx.shift = kShift;
    fx.k1 = to_i32((double)k1 * (kScale / 255.0));
    fx.k2 = to_i32((double)k2 * kScale);
    fx.k3 = to_i32((double)k3 * kScale);
    fx.k4 = to_i32((double)k4 * (kScale * 255.0));
...
{
    const int64_t rounder = int64_t(1) << (fx.shift - 1);
    const int64_t worst = (int64_t)std::abs(fx.k1) * 255 * 255 + rounder;
    return worst <= int64_t(2147483647);
}
...
{
    const int64_t rounder = int64_t(1) << (fx.shift - 1);
    const int64_t worst = (int64_t)std::abs(fx.k2) * 255 + rounder;
    return worst <= int64_t(2147483647);
}
    ...
    const ArithmeticCoeffFx& fx) noexcept
{
    ...
    for (; i + 4 <= n; i += 4)
    {
        ...
        int32x4_t ao = vmlaq_n_s32(kRound, a1, k2fx);
        int32x4_t ro = vmlaq_n_s32(kRound, r1, k2fx);
        int32x4_t go = vmlaq_n_s32(kRound, g1, k2fx);
        int32x4_t bo = vmlaq_n_s32(kRound, b1, k2fx);
    ...
    if (i < n)
        arithmetic_k2_only_prgb32_row_scalar(dst + i, s1 + i, n - i, fx);
}
#endif
...
{
    const int64_t rounder = int64_t(1) << (fx.shift - 1);
    const int64_t worst = (int64_t)std::abs(fx.k3) * 255 + rounder;
    return worst <= int64_t(2147483647);
}
    ...
    const ArithmeticCoeffFx& fx) noexcept
{
    ...
    for (; i + 4 <= n; i += 4)
    {
        ...
        int32x4_t ao = vmlaq_n_s32(kRound, a2, k3fx);
        int32x4_t ro = vmlaq_n_s32(kRound, r2, k3fx);
        int32x4_t go = vmlaq_n_s32(kRound, g2, k3fx);
        int32x4_t bo = vmlaq_n_s32(kRound, b2, k3fx);
    ...
    if (i < n)
        arithmetic_k3_only_prgb32_row_scalar(dst + i, s2 + i, n - i, fx);
}
#endif
...
static INLINE bool arith_k2_k3_can_use_neon(const ArithmeticCoeffFx& fx) noexcept
{
    const int64_t rounder = int64_t(1) << (fx.shift - 1);
    const int64_t worst =
        (int64_t)std::abs(fx.k2) * 255 +
        (int64_t)std::abs(fx.k3) * 255 +
        rounder;
...
static INLINE uint8_t arith_general_u8(
    uint8_t a,
    uint8_t b,
    const ArithmeticCoeffFx& fx) noexcept
{
...
static INLINE void copy_prgb32_row(
    uint32_t* dst,
    const uint32_t* src,
    size_t n) noexcept
{
