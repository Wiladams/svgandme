...
namespace waavs
{
    ...
    {
    public:
        WGRectD fViewportFrame{};
        WGRectD fViewBoxFrame{};
        double fRotRad = 0.0;		// Number of radians rotated
        WGPointD fRotCenter{};		// Point around which we rotate
        PreserveAspectRatio fPreserveAspectRatio;

    public:
        static void clampFrame(WGRectD& fr)
        {
            if (fr.w < 0) fr.w = 0;
            if (fr.h < 0) fr.h = 0;
        }
        ...
        void reset()
        {
            fViewportFrame = WGRectD{};
            fViewBoxFrame = WGRectD{};
            fRotRad = 0.0;
            fRotCenter = {};
            fPreserveAspectRatio = PreserveAspectRatio{};
        }

        void resetView(const WGRectD& viewBoxFr, const WGRectD& viewportFr)
        {
        ...
        void setViewportFrame(const WGRectD& fr) {
            fViewportFrame = fr;
            //clampFrame(fViewportFrame);
        }
        bool getViewportFrame(WGRectD& outFr) const
        {
        ...
        void setViewBoxFrame(const WGRectD& fr)
        {
            fViewBoxFrame = fr;
            //clampFrame(fViewBoxFrame);
        }
        bool getViewBoxFrame(WGRectD& outFr) const
        {
        ...
        void setRotation(double rads, const WGPointD& center)
        {
        ...
        bool getRotation(double& outRadians, WGPointD& outCenter) const
        {
            outRadians = fRotRad;
    ...
    // you can specify the viewboxFrame(WGRectD(0,0,img.width, img.height))
    // 
    ...
    public:
        ...
        ViewportTransformer(const WGRectD& aSurfaceFrame, const WGRectD& aSceneFrame)
        ...
        bool setViewportFrame(const WGRectD& fr)
        {
        ...
        bool getViewportFrame(WGRectD& aFrame) const
        {
            return fPortalView.getViewportFrame(aFrame);
        }

        // setting and getting scene frame
        bool setViewBoxFrame(const WGRectD& fr)
        {
            // return false if the frame is invalid
            if (fr.w <= 0 || fr.h <= 0)
                return false;
        ...
        bool getViewBoxFrame(WGRectD& aframe) const
        {
    ...
    protected:
        ...
        void getAspectScale(WGPointD& ascale)
        {
        ...
        void updateTransformMatrix()
        {
            ...
            fTransform.rotate(fPortalView.fRotRad, fPortalView.fRotCenter.x, fPortalView.fRotCenter.y);
    ...
    public:
        ...
        bool lookAt(double cx, double cy)
        {
            ...
            WGPointD sceneCenter = mapViewportToViewBox(surfaceCenterX, surfaceCenterY);
